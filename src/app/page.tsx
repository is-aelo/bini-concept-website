...
export default async function BiniPage() {
  ...
  const tours = [
    {
      event: "Summer Sonic 2026",
      city: "Tokyo",
      status: "Upcoming",
    },
    {
      event: "Signals World Tour",
      city: "Manila",
      status: "Upcoming",
    },
    {
  ...
  return (
    <div className="relative">
      ...
      <section className="bg-coachella py-32 px-10">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-20 items-center">
          <div className="space-y-6">
            <span className="text-label-mono bg-white/20 px-4 py-1 rounded-full">
              Historical Milestone
            </span>

            <h2 className="text-display-xl !text-6xl md:!text-8xl leading-none">
              COACHELLA 2026
            </h2>
          </div>
          ...
          <div className="flex flex-col justify-center space-y-6">
            <p className="text-xl font-medium leading-relaxed">
              Making history as the first-ever P-Pop group to perform at
              Coachella. A monumental leap for Filipino talent on the global
              stage.
            </p>

            <div className="h-[1px] w-full bg-current opacity-20"></div>

            <p className="text-label-mono font-bold italic">
              Mojave Stage • April 2026
            </p>
          </div>
        </div>
      </section>
      ...
      <section
        id="profile"
        className="py-32 px-10 bg-[var(--c-surface-2)]"
      >
        <h2 className="text-5xl mb-20 text-center font-serif italic lowercase">
          the members
        </h2>
          ...
          {members.map((member) => (
            ...
            >
              ...
              <div className="absolute inset-0 p-8 flex flex-col justify-end bg-gradient-to-t from-black/60 to-transparent">
                ...
                <h3 className="text-white text-3xl font-display uppercase tracking-tighter">
                  {member.stageName}
                </h3>

                <p className="text-label-mono text-white/70">
                  {member.fullName}
                </p>
              </div>
            </div>
      ...
      <section
        id="disco"
        className="py-32 bg-[var(--c-ink)] overflow-hidden"
      >
        <div className="max-w-7xl mx-auto px-10">
          <h2 className="text-display-xl !text-6xl text-[var(--c-surface)] mb-20">
            Discography
          </h2>
            ...
            {albums.map((album) => (
              <div key={album.title} className="group relative">
                <div className="cd-case relative flex flex-col items-center justify-center p-8 text-center text-[var(--c-surface)]">
                  <div className="absolute top-4 right-4 text-label-mono opacity-40">
                    {album.type}
                  </div>
                  ...
                  <p className="text-label-mono mt-4 opacity-60">
                    {album.year}
                  </p>
                </div>
              </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="space-y-4">
            {tours.map((tour, i) => (
              <div
                key={i}
                className="flex justify-between items-center py-8 border-b border-[var(--c-surface-3)] group"
              >
                <div className="space-y-1">
                  <h3 className="text-3xl font-display uppercase tracking-tighter group-hover:text-[var(--c-teal)] transition-colors">
                    {tour.event}
                  </h3>

                  <p className="text-label-mono opacity-60">
                    {tour.city}
                  </p>
                </div>

                <div
                  className={`px-6 py-2 rounded-full text-label-mono border ${
                    tour.status === "Upcoming"
                      ? "border-[var(--c-teal)] text-[var(--c-teal)]"
                      : "border-[var(--c-surface-3)] opacity-40"
                  }`}
                >
                  {tour.status}
                </div>
      ...
      <footer className="py-20 border-t border-[var(--c-surface-3)] flex flex-col items-center">
        <p className="text-label-mono opacity-50 mb-10">
          Designed for Portfolio Purposes • 2026
        </p>

        <h2 className="text-display-xl !text-[15vw] opacity-[0.03] select-none leading-none">
          BINICORE
        </h2>
      </footer>
    </div>
