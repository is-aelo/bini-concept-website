...
void main() {
  ...
  vec3 bottomRightColor = mix(uTeal, uTealDark, blobBR * 0.3);
...
function Scene() {
  ...
  const uniforms = useMemo(() => ({
    ...
    uTeal: { value: new THREE.Color(0x008080) },
    uTealDark: { value: new THREE.Color(0x004d4d) },
    uTealPale: { value: new THREE.Color(0xb2dfdb) },
  }), []);

  useEffect(() => {
    if (materialRef.current) {
      const style = getComputedStyle(document.documentElement);
      materialRef.current.uniforms.uTeal.value.set(style.getPropertyValue("--c-teal").trim());
      materialRef.current.uniforms.uTealDark.value.set(style.getPropertyValue("--c-teal-dark").trim());
      materialRef.current.uniforms.uTealPale.value.set(style.getPropertyValue("--c-teal-pale").trim());
    }
  }, []);
